...
static void *pControllerDataBase ( void *arg );
...
int main ( void )
{
  ...
  pthread_t   cntrllrDataBase_tid;        /* Controller Data Base tid */
  pthread_t   database_tid;               /* Database tid */
  ...
  pthread_create ( &cntrllrDataBase_tid, NULL, pControllerDataBase, NULL );
  ...
  pthread_join ( cntrllrDataBase_tid, NULL );
...
static void *pComm ( void *arg )
{
  ...
  printf ( "Huerta/Garden server waiting on port %d\n", SERVER_PORT );
  ...
  printf ( "\nConnected from Huerta/Garden TUI @ %s, port %d\n\n",
